...
ggplot(chunk2Data, aes(x = Cost, y = CPA, size = Conversions)) +
    geom_point() +
...
ggplot(chunk3Data, aes(x = Cost, y = CPA, size = Conversions)) +
    geom_point() +
    geom_text(aes(label = Campaign))
...
names(chunk4Data) <- c("Campaign", "Conversions", "CPA", "Cost")
 ...
 ggplot(chunk4Data, aes(x = Cost, y = CPA, size = Conversions)) +
    geom_point() +
    geom_text(aes(label = Campaign))
...
chunk12Data %>% group_by(Adrelevance) %>% summarize(n = n()) %>% mutate(freq = n/sum(n)) %>% arrange(desc(n))
chunk12Data %>% group_by(Landingpageexperience) %>% summarize(n = n()) %>% mutate(freq = n/sum(n)) %>% arrange(desc(n))
chunk12Data %>% group_by(Expectedclickthroughrate) %>% summarize(n = n()) %>% mutate(freq = n/sum(n)) %>% arrange(desc(n))
chunk12Data %>% group_by(Qualityscore) %>% summarize(n = n()) %>% mutate(freq = n/sum(n)) %>% arrange(desc(n))
...
```{r geoperformance, echo=FALSE, eval=FALSE}

counties <- readOGR("misc","Census2011_Admin_Counties_generalised20m")
locationIds <- read.csv("misc/AdWords_API_Location_Criteria.csv")
names(locationIds) <- c("id","name","longName","parentId","countryCode","targetType","status")
