...
chunk19_body <- statement(select=c("CountryCriteriaId","RegionCriteriaId","Cost","Conversions","AverageCpc", "CostPerConversion"),
                         report="GEO_PERFORMANCE_REPORT",
                         start=ninetyDaysAgo,
                         end=yesterday)

chunk19_data <- getData(clientCustomerId=customerId, google_auth=googleAuth, statement=chunk19_body)
names(chunk19_data) <- c("Country","Region","Cost","Conversions","CPC","CPA")
chunk19_data$Region <- as.numeric(chunk19_data$Region)

mergedAdWords <- inner_join(chunk19_data, locationIds, by = c("Region"="id"))
